...
export default function StudentDashboard({ userId }: { userId: string }): React.ReactElement {
    ...
    const renderHomeTab = () => (
        <>
            ...
            <View style={[styles.sectionContainer, isDesktop && styles.desktopSectionContainer]}>
                ...
                {/* Course Items */}
                <View style={isDesktop ? styles.desktopCoursesGrid : undefined}>
                    {courses.map((course) => (
                        <TouchableOpacity
                            key={course._id}
                            style={[styles.courseItem, isDesktop && styles.desktopCourseItem]}
            ...
            <View style={[styles.sectionContainer, isDesktop && styles.desktopSectionContainer]}>
                ...
                {/* Assignment Items */}
                <View style={isDesktop ? styles.desktopAssignmentsGrid : undefined}>
                    {assignments.slice(0, isDesktop ? 6 : 3).map((assignment) => (
    ...
    const renderAttendanceTab = () => (
        <View style={[styles.tabContent, isDesktop && styles.desktopTabContent]}>
            ...
            <View style={[styles.attendanceOverview, isDesktop && styles.desktopAttendanceOverview]}>
            ...
            <View style={isDesktop ? styles.desktopAttendanceGrid : undefined}>
                {courses.map((course) => {
                    ...
                    return (
                        <TouchableOpacity
                            key={course._id}
                            style={[styles.attendanceCourseItem, isDesktop && styles.desktopAttendanceCourseItem]}
                            onPress={() => {
                                setSelectedCourse(course)
    ...
    const renderSidebar = () => {
        if (isSmallDevice && !sidebarOpen) return null

        return (
            <View
                style={[
                    styles.sidebar,
                    isSmallDevice && sidebarOpen && styles.mobileSidebar,
                    !isSmallDevice && !sidebarOpen && styles.collapsedSidebar,
                ]}
            >
                <View style={styles.sidebarHeader}>
                    <Text style={styles.sidebarLogo}>EduConnect</Text>
                    {!isSmallDevice && (
                        <TouchableOpacity style={styles.sidebarToggle} onPress={() => setSidebarOpen(!sidebarOpen)}>
                            <Ionicons name={sidebarOpen ? "chevron-back" : "chevron-forward"} size={24} color="white" />
                        </TouchableOpacity>
                    )}
                </View>
                ...
                <View style={styles.sidebarContent}>
                    <TouchableOpacity
                        ...
                        onPress={() => {
                            setActiveTab("home")
                            if (isSmallDevice) setSidebarOpen(false)
                        }}
                    >
                    ...
                    <View style={styles.desktopMainContent}>
                        ...
                        <View style={styles.desktopHeader}>
                            ...
                            <View style={styles.profileContainer}>
                                ...
                                >
                                    <View style={styles.desktopProfilePic}>
                                        <Ionicons name="person" size={24} color="#5c51f3" />
                                    </View>
                                    <Text style={styles.desktopProfileName}>{displayName}</Text>
                                    <Ionicons
                                ...
                                {isProfileMenuVisible && (
                                    <View style={styles.desktopProfileDropdown}>
